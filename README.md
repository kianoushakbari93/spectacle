...
Spectacle builds natively for Apple Silicon (arm64) as well as Intel (the
resulting binary is a universal `arm64` + `x86_64` Mach-O). The app has **no
external runtime dependencies** and no auto-update mechanism. The only build
requirement is **full Xcode** - the Command Line Tools alone are not sufficient,
since `xcodebuild` is required.

### 1. Build

Open the project and run with &#8984;R:

    $ open Spectacle.xcodeproj

…or build from the command line:

    $ xcodebuild -project Spectacle.xcodeproj -scheme Spectacle -configuration Debug build

> Note: the legacy test-only dependencies (Specta/Expecta/OCHamcrest/OCMockito)
> no longer compile on current toolchains. They are not linked by the app, so
> the `SpectacleSpecs` test target is not built by default.

### 2. Find the built app

A command-line `xcodebuild` build writes the app into Xcode's DerivedData, not
into the project folder. Locate it with:

    $ find ~/Library/Developer/Xcode/DerivedData/Spectacle-*/Build/Products/Debug -maxdepth 1 -name Spectacle.app

(`Debug` becomes `Release` for a `-configuration Release` build. The
`Spectacle-xxxxxxxx` folder name is generated by Xcode, so don't hardcode it.)

If you build inside Xcode instead, right-click the `Spectacle.app` product in the
Project navigator and choose *Show in Finder*.

### 3. Install and run

Copy the app into `/Applications` (quit any running copy first):

    $ APP=$(find ~/Library/Developer/Xcode/DerivedData/Spectacle-*/Build/Products/Debug -maxdepth 1 -name Spectacle.app | head -1)
    $ pkill -x Spectacle; cp -R "$APP" /Applications/Spectacle.app
    $ open -a Spectacle

Spectacle is a menu-bar app (`LSUIElement`), so it appears in the status bar
rather than the Dock; its window-management features and preferences are reached
from that status-bar menu.

**Grant Accessibility access.** Spectacle moves windows through the macOS
Accessibility API, so it does nothing until you allow it in **System Settings →
Privacy & Security → Accessibility** (the app prompts you on first launch). The
app is ad-hoc signed, so its signature changes on every rebuild; after
reinstalling a new build, remove any stale **Spectacle** entry from that list and
re-enable the freshly installed one.

### 4. Verify a native arm64 build

Confirm the produced binary contains an `arm64` slice:

    $ lipo -archs "$(find ~/Library/Developer/Xcode/DerivedData -name Spectacle -path '*/Build/Products/*/Spectacle.app/Contents/MacOS/Spectacle' | head -1)"
